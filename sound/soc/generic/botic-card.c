        ...
        snd_pcm_format_t format, struct botic_ser_setup *ser_setup)
{
    ...
    if (n_spdif > 0) {
#if 1
        ser_setup->dai_fmt = SND_SOC_DAIFMT_I2S;
#else
...
static int asoc_botic_card_probe(struct platform_device *pdev)
{
    ...
    botic_dai.codecs->of_node = of_parse_phandle(np, "audio-codec", 0);
    if (botic_dai.codecs->of_node) {
        ret = of_property_read_string_index(np, "audio-codec-dai", 0,
                &botic_dai.codecs->dai_name);
        if (ret < 0) {
            goto asoc_botic_card_probe_error;
        }
    } else {
    ...
    botic_dai.cpus->of_node = of_parse_phandle(np, "audio-port", 0);
    if (!botic_dai.cpus->of_node) {
        ret = -ENOENT;
        goto asoc_botic_card_probe_error;
    }

    /* TODO */
    botic_dai.platforms->of_node = botic_dai.cpus->of_node;
