...
static int sabre32_codec_remove(struct snd_soc_component *component)
{
    struct sabre32_codec_data *codec_data = snd_soc_codec_get_drvdata(component);
...
static int asoc_sabre32_codec_probe(struct platform_device *pdev)
{
    ...
    struct sabre32_data *codec_data;
    int ret;
    ...
    codec_data = devm_kzalloc(&pdev->dev, sizeof(struct sabre32_data),
            GFP_KERNEL);
